...
| response_mode | 含义 |
|---|---|
| `raw_list` | 精准定位，只输出原始列表 |
| `list_with_insights` | 列表 + 少量见解 |
| `report` | 结构化综述报告 |
...
## 运行逻辑链路
  ...
  agent/agent.py
    ...
    brain.py · route()
      ...
      └─ 输出 RoutingDecision
           {search_mode, query_text, top_k,
            title, authors, categories, comment, published,
            response_mode}
          │
          ▼
    agent.py · _search(routing)
      ...
      └─ hybrid    → PaperSearcher.hybrid_search()
          ...
          │
          ▼
    agent.py · build_render_prompt()
      └─ 拼接 user_input + routing + results(精简版)
          │
          ▼
    agent.py · renderer (MoonShot)
